...
import TablePopover from "src/components/table/table-menu";
...
export default function TransactionView() {
  ...
  const notFound = transactions.length === 0 && !!filterName;
  ...
  const handleCompanySelect = useCallback(
    (companyId) => {
      setSelectedCompany((prevSelectedCompany) => {
        const selectedCompan = companies.find(
        ...
        const filteredUsers = users.map((user) => ({
          ...
          isApplied: transactions.some(
            (transaction) =>
              ...
              transaction.company._id === selectedCompan._id
          ),
        }));
  ...
  const handleSortChange = async (option) => {
    ...
    try {
      const transactionsSummary =
        await transactionService.getTransactionsByCompany(option.value);
  ...
  const handleExportTransactions = () => {
    ...
    doc.text(
      ...
      doc.internal.pageSize.width / 2.4,
      20,
      { align: "center" }
    );
  ...
  return (
    <Container maxWidth="xl">
      <Stack direction="row" alignItems="left" justifyContent="space-between" mb={3}
        ...
        <Typography variant="h4" sx={{ textAlign: 'centre', mb: 1 }}>{PAGE_TITLES.TRANSACTIONS}</Typography>
        <Stack direction="row" spacing={0} alignItems='end'
          sx={{ justifyContent: 'flex-end', }}>
          <TablePopover
            actions={[
              { label: 'Status', icon: 'eva:checkmark-circle-2-fill', method: () => setOpenCompanyModal(true) },
              { label: 'New', icon: 'eva:plus-fill', method: () => setOpenCreateDialog(true) },
              { label: 'Export', icon: 'eva:file-outline', method: () => handleExportTransactions() },
            ]}
          />
        </Stack>
      </Stack>
