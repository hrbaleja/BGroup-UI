...
import {
  Box, List, Avatar, Divider, Popover, Typography, IconButton, ListItemText,
  ListItemAvatar, ListSubheader, ListItemButton, Collapse, Button, Paper, Grid, Dialog, DialogTitle,
  DialogContent,
...
// Generate dummy contacts and group by first letter
const CONTACTS = [...Array(20)].map(() => {
...
const generateDummyContacts = () => {
  const firstNames = ['John', 'Sarah', 'Michael', 'Emma', 'David', 'Lisa', 'James', 'Anna', 'Robert', 'Maria', 'William', 'Jessica', 'Daniel', 'Ashley', 'Christopher'];
...
export default function ContactBookPopover() {
  ...
  const [contacts, setContacts] = useState(generateDummyContacts());
  ...
  const handleAddContact = () => {
    if (newContact.name.trim()) {
      const contact = {
        id: Date.now(),
        ...newContact,
        avatar: `https://api.dicebear.com/7.x/avataaars/svg?seed=${newContact.name.replace(' ', '')}&backgroundColor=b6e3f4,c0aede,d1d4f9,ffd5dc,ffdfbf`
      };
  ...
  return (
    <>
      ...
      >
        ...
        <Divider />
        <Box sx={{ p: 1 }}>
          <Button fullWidth>View All Contacts</Button>
        </Box>
      </Popover>
    </>
  );
