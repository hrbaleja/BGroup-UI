...
export default function AccountFilters({ openFilter, onOpenFilter, onCloseFilter, onFilter }) {
  ...
  const handleBalanceTypeChange = (event) => {
    ...
    let updatedBalanceTypes;

    if (value === 'Zero') {
      updatedBalanceTypes = checked ? ['Zero'] : [];
      setAmount('');
      setAmountDisabled(checked);
    } else {
      updatedBalanceTypes = checked ? [value] : [];
      setBalanceTypes(updatedBalanceTypes);
          ...
          <FormControlLabel
            ...
            control={
              <Checkbox
                ...
                value={item}
                disabled={
                  item === 'Zero' && balanceTypes.includes('Zero') && !balanceTypes.includes(item)
                }
              />
            }
            label={item}
